...
@userRouter.patch('/update_warns')
def update_warns_function(current_user_hwid: str = Body(embed=True), adb_id: str = Body(embed=True), cluster_name: str = Body(embed=True), username: str = Body(embed=True), warn_type: str = Body(embed=True), db: Session = Depends(get_db)):
    user_hwid = query_tiktok_table_check_auth(current_user_hwid)
    if user_hwid is None:
        raise HTTPException(status_code=311, detail="Autentication failed")
    else:
        current_warns = db.query(func.coalesce(func.sum(model.TikTokTable.warns), 0)).filter(
        model.TikTokTable.user_id == username,
        model.TikTokTable.adb_address == adb_id,
        model.TikTokTable.cluster_name == cluster_name,
        ).scalar()

        updated_warns = current_warns + 1
        if warn_type == 'add_warn':
            response = db.query(model.TikTokTable).filter(
                model.TikTokTable.user_id == username,
                model.TikTokTable.adb_address == adb_id,
                model.TikTokTable.cluster_name == cluster_name,
            ).update({model.TikTokTable.warns: updated_warns}, synchronize_session=False)

            db.commit()
	
            return {"added_+1_warn": response}
        
        if warn_type == 'reset_warns':
            response = db.query(model.TikTokTable).filter(
            model.TikTokTable.user_id == username,
            model.TikTokTable.adb_address == adb_id,
            model.TikTokTable.cluster_name == cluster_name,
            ).update({model.TikTokTable.warns: 0}, synchronize_session=False)

            db.commit()
            
            return {"warns_reseted_to_0": response}
